    ...
    career: {
        ...
        resume: {
            builder: {
                ...
                languageHint: 'Предпочтительный язык результата',
                marketLabel: 'Рынок вакансий',
                marketHint: 'Где вы хотите работать',
                workModeLabel: 'Формат работы',
                workModeHint: 'Как вы хотите работать',
                ...
                markets: {
                    local: 'Локально - Кыргызстан',
                    central_asia: 'Центральная Азия',
                    russian_speaking: 'Русскоязычный рынок',
                    eu: 'Европейский союз',
                    us: 'США',
        ...
        jobs: {
            ...
            gating: {
                title: 'Завершите резюме, чтобы открыть персональные действия',
                description: 'Смотреть реальные вакансии можно уже сейчас. Оценка совпадения, адаптация резюме и генерация сопроводительного письма откроются, когда резюме будет готово.',
                noResume: 'Выберите или создайте резюме, чтобы увидеть персональные рекомендации по вакансии.',
                incompleteResume: 'Добавьте более точную роль, достаточно навыков и доведите готовность резюме до 70+, чтобы открыть персональные действия.',
                marketLabel: 'Фокус рынка',
                workModeLabel: 'Формат работы',
                markets: {
                    local: 'Локально - Кыргызстан',
                    central_asia: 'Центральная Азия',
                ...
                scoreLocked: 'Оценка совпадения откроется после готовности резюме.',
                actionLocked: 'Адаптация, сопроводительное письмо и интервью-подготовка откроются после готовности резюме.',
            },
            interviewPrep: {
            ...
            detail: {
                workspaceTitle: 'Рабочая зона отклика',
                workspaceSubtitle: 'Используйте одно сохранённое резюме, чтобы адаптировать, откликаться и готовиться в одном месте.',
                resumeLabel: 'Резюме',
                applyExternal: 'Откликнуться на сайте компании',
                preferredSkills: 'Желательные навыки',
                noDescription: 'Описание не указано.',
                status: 'Статус отклика',
                notApplied: 'Ещё не откликались',
                coverLetterTitle: 'Сопроводительное письмо прямо здесь',
                coverLetterSubtitle: 'Сгенерируйте черновик под вакансию, не покидая страницу.',
                coverLetterTone: 'Тон',
                coverLetterLength: 'Объём',
                coverLetterEmpty: 'Сгенерируйте сопроводительное письмо, и оно появится здесь для редактирования.',
                coverLetterSuccess: 'Сопроводительное письмо создано.',
                interviewTitle: 'Краткая подготовка к интервью',
                interviewSubtitle: 'Используйте текущее резюме и эту вакансию, чтобы подготовиться к следующему разговору.',
                interviewOpen: 'Открыть полную подготовку',
                tailorSuccess: 'Адаптированное резюме создано.',
                applySuccess: 'Отклик создан.',
                saveSuccess: 'Вакансия сохранена.',
                unsaveSuccess: 'Вакансия удалена из сохранённых.',
                noResumeAction: 'Сначала создайте или выберите резюме.',
            },
        },
        signup: {
            prompt: {
