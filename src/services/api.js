...
export const unenrollUserFromCourse = async (userId, courseId) => {
	try {
		const response = await api.delete(
			`/enrollments/${courseId}/unenroll/${userId}`
		);
		return response.data;
	} catch (error) {
		console.error('Error enrolling user:', error);
...
export const fetchCourses = async ({
	...
	try {
		const response = await api.get('/courses', {
			params: clean({ q, limit, excludeIds }),
		});
		return response.data;
	} catch (error) {
...
export async function fetchLessonChallenge(
	courseId,
	sectionId,
	lessonId,
	manage = false
...
export async function upsertLessonChallenge(
	courseId,
	sectionId,
	lessonId,
	challengeData
...
export async function submitLessonChallenge(
	courseId,
	sectionId,
	lessonId,
	payload
...
export const updateLastViewedLesson = async ({ courseId, lessonId }) => {
	const res = await api.put(`/student-progress/last-viewed`, {
		courseId,
		lessonId,
	});
	return res.data;
...
export const getLastViewedLesson = async (courseId) => {
	const res = await api.get(
		`/student-progress/last-viewed?courseId=${courseId}`
	);
	return res.data;
};
...
export const getVideoTime = async (courseId, lessonId) => {
	const res = await api.get(
		`courses/${courseId}/progress/lessons/${lessonId}/video-time`
	);
	return res.data;
};
...
export async function listOfferingsForCourse(courseId, { q } = {}) {
	const { data } = await api.get(`/offerings/course/${courseId}`, {
		params: { q },
	});
...
export const assignCourseToCompany = async (courseId, companyId) => {
	const { data } = await api.post(
		`/courses/${courseId}/companies/${companyId}`
	);
	return data;
};
...
export async function addCompanyMember(
	companyId,
	payload /* { userId:number, role:'company_admin'|'instructor'|'assistant' } */
) {
