...
export const LeaderboardHero = ({
    ...
    return (
        <section className="relative overflow-hidden rounded-[28px] border border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(251,146,60,0.28),_transparent_34%),linear-gradient(135deg,_#0f172a_0%,_#111827_52%,_#1d4ed8_100%)] p-6 text-white shadow-[0_24px_80px_-36px_rgba(15,23,42,0.85)] sm:p-8">
            <div className="absolute inset-y-0 right-0 w-56 bg-[radial-gradient(circle,_rgba(255,255,255,0.22),_transparent_62%)]" />
            <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
                <div className="max-w-2xl space-y-4">
                    <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-orange-100">
                ...
                <div className="grid w-full max-w-xl grid-cols-1 gap-3 sm:grid-cols-3">
                    ...
                    <div className="rounded-[22px] border border-white/15 bg-white/10 p-4 backdrop-blur">
                        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-200">Эми эмне кылуу керек</p>
                        <p className="mt-2 text-lg font-semibold leading-tight">{nextActionTitle}</p>
                        <p className="mt-1 text-sm text-slate-200">{nextActionHint}</p>
...
const HeroMetricCard = ({ label, value, helper }) => (
    <div className="rounded-[22px] border border-white/15 bg-white/10 p-4 backdrop-blur">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-200">{label}</p>
        <p className="mt-2 text-2xl font-semibold text-white">{value}</p>
        <p className="mt-1 text-sm text-slate-200">{helper}</p>
    </div>
);

export const LeaderboardListCard = ({ title, description, items, currentUserId = null, footer = null }) => (
    <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_-42px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-[#161b22] sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="mt-1 flex flex-wrap gap-3 text-sm text-slate-500 dark:text-slate-300">
...
export const NearYouRail = ({ items = [], currentUserId = null, targetGap = null, nextTargetName = '' }) => {
    const normalizedItems = Array.isArray(items) ? items : [];
    ...
    return (
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_-42px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-[#161b22] sm:p-6">
            <div className="flex items-start justify-between gap-4">
                <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-orange-500">Жакынкы орундар</p>
                    <h3 className="mt-2 text-lg font-semibold text-slate-900 dark:text-white">Сизге эң жакын атаандаштар</h3>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">Кийинки орунга чыгуу үчүн кимди өтүү керек экенин ушул жерден көрөсүз.</p>
                ...
                <div className="rounded-2xl border border-orange-200 bg-orange-50 px-3 py-2 text-right text-xs font-semibold text-orange-700 dark:border-orange-500/30 dark:bg-orange-500/10 dark:text-orange-200">
...
export const MySkillProgressGrid = ({ items = [] }) => (
    <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_-42px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-[#161b22] sm:p-6">
        <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 to-sky-500 text-white shadow-lg shadow-cyan-200/60 dark:shadow-none">
                <FiTarget />
            </div>
            <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Менин көндүм прогрессим</h3>
                <p className="text-sm text-slate-500 dark:text-slate-300">Бул блок жеке өздөштүрүү прогрессин көрсөтөт. Рейтинг болсо төмөндө өзүнчө турат.</p>
            </div>
        </div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 2xl:grid-cols-3">
            {items.length ? (
                items.map((item) => (
                    <article key={item.id || item.slug || item.name} className="rounded-[22px] border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-900/60">
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                <p className="text-base font-semibold text-slate-900 dark:text-white">{item.name}</p>
                                <p className="mt-1 text-xs uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">{item.progressPercent || 0}% өздөштүрүү</p>
                            </div>
                            <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-700 shadow-sm dark:bg-slate-950/70 dark:text-slate-200">
                                {item.xp || 0} XP
                            </span>
                        </div>
...
export const AchievementCloud = ({ items = [], title = 'Жетишкендиктер', subtitle = 'Көрүнүп турган сыйлыктар', shareMeta = null }) => (
    <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_-42px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-[#161b22] sm:p-6">
        <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-orange-500 to-amber-400 text-white shadow-lg shadow-orange-200/60 dark:shadow-none">
                <FiAward />
            </div>
            <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white">{title}</h3>
            ...
            {(items.length ? items : [{ title: 'Алгачкы кадам' }, { title: 'Ылдам башталыш' }, { title: 'Тест жаркылыгы' }]).map((item, index) => {
                ...
                return (
                    <article
                        ...
                        className={`flex h-full min-h-[320px] flex-col rounded-[24px] border bg-gradient-to-br ${accentSets[index % accentSets.length]} p-5`}
                    >
                        <div className="flex items-start justify-between gap-3">
                            <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/85 text-slate-900 shadow-sm dark:bg-slate-900/70 dark:text-white">
                                <FiAward />
                            </span>
                            <span className="rounded-full bg-white/85 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-700 dark:bg-slate-950/70 dark:text-slate-200">
                                {rarityLabel}
                            </span>
                        </div>
                        <div className="mt-5 space-y-2">
                            <p className="line-clamp-2 text-xl font-semibold leading-tight text-slate-900 dark:text-white">{shareTitle}</p>
                            <p className="line-clamp-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
                                {shareDescription}
                            </p>
                        </div>
                        <div className="mt-auto pt-5">
                            <div className="mb-4 space-y-2 border-t border-white/70 pt-4 dark:border-slate-800/70">
...
export const PublicSpotlightPanel = ({ studentOfWeek = null, highlights = [], metrics = [] }) => {
    ...
    return (
        <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-[linear-gradient(135deg,_#fff7ed_0%,_#ffffff_50%,_#eff6ff_100%)] p-4 shadow-[0_18px_60px_-42px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-[#161b22] sm:p-6">
                ...
                <div className="space-y-4">
                    <div className="grid gap-3 sm:grid-cols-3">
                        {(metrics.length
                            ...
                            <div key={`${metric.label}-${index}`} className="rounded-[20px] border border-slate-200 bg-white/85 p-4 dark:border-slate-700 dark:bg-slate-900/60">
                                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">{metric.label}</p>
                    ...
                    <div className="rounded-[24px] border border-orange-200 bg-white/80 p-5 dark:border-orange-500/20 dark:bg-slate-900/60">
                        <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div className="min-w-0">
                                ...
                                <div className="mt-4 flex items-center gap-4">
                                    ...
                                    <div className="min-w-0">
                                        <p className="truncate text-xl font-semibold text-slate-900 dark:text-white">{studentOfWeek?.fullName || 'Жаңы лидер күтүлүүдө'}</p>
                            ...
                            <span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-orange-700 dark:bg-orange-500/10 dark:text-orange-200">Өсүүдө</span>
                        </div>
                    </div>
                </div>
...
export const ChallengeRail = ({ items = [] }) => (
    <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_-42px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-[#161b22] sm:p-6">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Жакынкы максаттар</h3>
                <p className="text-sm text-slate-500 dark:text-slate-300">Эң аз аракет менен эң көп өсүш берген кадамдар</p>
            </div>
        </div>
        <div className="mt-5 space-y-3">
            {(items.length
                ...
                return (
                    ...
                    >
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                {highlighted ? <p className="text-xs font-semibold uppercase tracking-[0.18em] text-orange-500">Эң пайдалуу кадам</p> : null}
                                <p className={`font-semibold ${highlighted ? 'mt-1 text-lg text-slate-900 dark:text-white' : 'text-slate-900 dark:text-white'}`}>{item.title}</p>
                                <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">{item.detail || item.value}</p>
                            </div>
                            <FiArrowRight className={`mt-1 shrink-0 ${highlighted ? 'text-orange-500' : 'text-slate-400'}`} />
                        </div>
                        {target > 0 ? (
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2">
                            {item.reward ? (
                                <div className="inline-flex rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-700 dark:border-orange-500/30 dark:bg-orange-500/10 dark:text-orange-200">
                                    Сыйлык: {item.reward}
                                </div>
                            ) : null}
                        ...
                        <div className="mt-4">
                            <Link
                                to={action.to}
                                className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950/60 dark:text-slate-100 dark:hover:bg-slate-900"
                            >
                                <ActionIcon className="shrink-0" />
                                {action.label}
...
export const SkillSpotlightGrid = ({ boards = [], personalProgress = [], featuredSlug = '' }) => {
    ...
    return (
        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_18px_60px_-42px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-[#161b22] sm:p-6">
            <div className="flex items-center justify-between gap-3">
                <div>
                    <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Көндүм фокусу</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-300">Ар бир багытта лидер ким жана сизге кийинки өсүү чекити кайда экенин көрсөтөт.</p>
                </div>
                    ...
                    cards.map((board, index) => {
                        ...
                        return (
                            <article
                                ...
                                className={`rounded-[22px] border bg-gradient-to-br ${accentSets[index % accentSets.length]} p-4`}
                            >
                                <div className="flex items-center justify-between gap-3">
                                    <div className="min-w-0">
                                        <div className="flex flex-wrap items-center gap-2">
                                            <p className="text-base font-semibold text-slate-900 dark:text-white">{board.label}</p>
                                            {isFeatured ? <span className="rounded-full bg-slate-900 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-white dark:bg-white dark:text-slate-900">Тандалган багыт</span> : null}
                                        </div>
                                    ...
                                    <span className="rounded-full bg-white/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-700 dark:bg-slate-950/70 dark:text-slate-200">
                                        #{leader ? 1 : '-'}
                                    </span>
                                </div>
                                ...
                                <div className="mt-4 overflow-hidden rounded-full bg-white/70 dark:bg-slate-950/50">
                                    <div
                                ...
                                <div className="mt-3 flex items-center justify-between gap-3 text-sm text-slate-600 dark:text-slate-300">
                                    ...
                                    {xpGap && xpGap > 0 ? <span className="font-semibold text-cyan-700 dark:text-cyan-300">+{xpGap} XP</span> : null}
                                </div>
                                <p className="mt-3 text-sm leading-6 text-slate-700 dark:text-slate-200">{nextHint}</p>
                            </article>
                        );
                    })
                ) : (
