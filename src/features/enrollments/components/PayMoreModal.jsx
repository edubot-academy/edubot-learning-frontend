...
const PayMoreModal = ({ student, onClose, onSuccess }) => {
    ...
    const handlePay = async () => {
        ...
        if (Number(amount) > maxAmount) {
            toast.error(
                `Кошумча сумма чектен ашып кетти. Калган сумма: ${formatCurrency(maxAmount)}`
            );
            return;
        }
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg relative">
                ...
                {selectedCourseId && (
                    <div className="text-sm text-gray-600 mb-4">
                        Жалпы баа:{' '}
                        {formatCurrency(
                            unpaidCourses.find((c) => c.id === Number(selectedCourseId))?.totalFee
                        )}
                        <br />
                        Төлөнгөнү:{' '}
                        {formatCurrency(
                            unpaidCourses.find((c) => c.id === Number(selectedCourseId))?.paid
                        )}
                        <br />
                        Калган сумма:{' '}
                        {formatCurrency(
                            unpaidCourses.find((c) => c.id === Number(selectedCourseId))?.remaining
                        )}
                    </div>
                )}
