...
import {
    listCompanyMembers, addCompanyMember, removeCompanyMember, setCompanyMemberRole, fetchUsers
} from '../../services/api';
...
export default function CompanyMembers({ companyId }) {
    ...
    const load = async () => {
        try { setItems(await listCompanyMembers(companyId)); }
        catch { toast.error('Мүчөлөрдү жүктөөдө ката.'); }
    };
        ...
        const t = setTimeout(async () => {
            ...
            try {
                ...
                const cleaned = arr
                    ...
                    .map(u => ({ id: u.id, fullName: u.fullName, email: u.email, avatarUrl: u.avatarUrl }));
                setResults(cleaned);
    ...
    const onAdd = async (e) => {
        ...
        try {
            ...
            setSelected(null); setQ(''); setRole('instructor');
            await load();
        } catch {
    ...
    const onRemove = async (memberId) => {
        ...
        try { await removeCompanyMember(companyId, memberId); toast.success('Өчүрүлдү.'); load(); }
                    ...
                    <div className="relative">
                        ...
                        {(results.length > 0 || searching) && (
                            <div className="absolute z-10 mt-1 w-full bg-white border rounded shadow max-h-72 overflow-auto">
                                {searching && <div className="px-3 py-2 text-sm text-gray-500">Изделүүдө…</div>}
                                ...
                                {!searching && results.length === 0 && (
                                    <div className="px-3 py-2 text-sm text-gray-500">Туура келген колдонуучу жок.</div>
                                )}
                            </div>
                        )}
                    </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                            ...
                            <tr key={m.id} className="border-b">
                                <td className="p-2">
                                    <div className="flex items-center gap-2">
                                        {m.avatarUrl ? (
                                            <img src={m.avatarUrl} className="h-6 w-6 rounded-full object-cover" />
                                        ) : <div className="h-6 w-6 rounded-full bg-gray-200" />}
                        ...
                        {items.length === 0 && (
                            <tr><td colSpan="4" className="p-4 text-center text-gray-500">Мүчөлөр табылган жок.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
