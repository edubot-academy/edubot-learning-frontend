...
export default function CompanyList() {
    ...
    React.useEffect(() => {
        (async () => {
            ...
            catch { toast.error('Компанияларды жүктөөдө ката.'); }
        })();
    }, [page, q]);
    ...
    return (
        <div className="max-w-5xl mx-auto p-4 space-y-6">
            <div className="flex items-center justify-between gap-3">
                ...
                <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Издөө…" className="border rounded px-3 py-1" />
            </div>

            <form onSubmit={onCreate} className="flex gap-2">
                <input className="border rounded px-3 py-2 w-full" placeholder="Жаңы компаниянын аты"
