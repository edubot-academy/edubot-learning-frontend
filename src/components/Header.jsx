...
const NavLinks = ({ user, onClick, textColor }) => {
  ...
  const isActive = (path) =>
    location.pathname === path ? "text-orange-400" : textColor;
  ...
  return (
    <>
      <Link
        to="/courses"
        onClick={onClick}
        className={`${isActive("/courses")} hover:text-orange-400 transition`}
      >
        Курстар
      </Link>
      <Link
        to="/about"
        onClick={onClick}
        className={`${isActive("/about")} hover:text-orange-400 transition`}
      >
        Биз жөнүндө
      </Link>
      <Link
        to="/contact"
        onClick={onClick}
        className={`${isActive("/contact")} hover:text-orange-400 transition`}
      >
        Байланыш
      </Link>
      ...
      {user?.role === "admin" && (
        <Link
          to="/admin"
          onClick={onClick}
          className={`${isActive("/admin")} hover:text-orange-400 transition`}
        >
          Админ
        </Link>
      )}
...
const Header = ({ cart = [] }) => {
  ...
  // Определяем, главная ли страница
  const isHomePage = location.pathname === "/";
  const textColor = isHomePage ? "text-white" : "text-[#122144]";
  const bgColor = isHomePage ? "bg-[#003A45]" : "bg-white";
  ...
  return (
    <header
      ref={headerRef}
      className={`fixed w-full z-30 ${bgColor} transition-colors duration-300`}
    >
      <div className="w-full px-8 py-6 flex justify-between">
        <div className="flex items-center gap-[60px]">
          <Link to="/" className="flex items-center space-x-3">
            ...
            <div translate="no" className="flex flex-col">
              ...
              <span
                className={`text-base tracking-widest font-medium ${textColor}`}
              >
                LEARNING
              </span>
            </div>
        ...
        <div className="flex items-center lg:gap-[33px] md:gap-2 gap-[20px]">
            ...
            {user ? (
              ...
              <div className="flex items-center gap-[8px] lg:gap-[28px]">
                ...
                <div className="lg:space-x-4 space-x-1">
                  ...
                  <Link
                    to="/register"
                    className="bg-orange-500 text-white px-6 py-3.5 rounded-full hover:bg-orange-600 transition"
                  >
                    Катталуу
                  </Link>
                </div>
          ...
          <GrLanguage
            ...
            className={`${textColor} w-[23px] h-[23px] cursor-pointer hover:text-orange-400`}
          />
        </div>
      </div>
      {menuOpen && (
        <div
          ref={mobileMenuRef}
          className="md:hidden bg-[#1c3a3e] shadow-md py-4 flex flex-col items-center space-y-4 w-full mobile-menu transition-all duration-300 transform scale-100 opacity-100"
        >
          {user && (
            <Link
              to="/profile"
          ...
          {!user ? (
            <div className="flex flex-col space-y-2 w-full px-6">
              <Link
                ...
                className="text-white hover:text-orange-400 transition font-semibold text-lg text-center"
              >
                Логин
              </Link>
              <Link
      ...
      {languageMenuOpen && (
        <div
          ref={languageMenuRef}
          className="absolute bg-orange-500 w-[50px] top-[100px] right-[20px] grid text-center py-1 rounded-lg"
        >
          <strong
            onClick={() => setLanguageMenuOpen(false)}
            className="text-white cursor-pointer hover:text-[#073034] transition"
          >
            KG
          </strong>
          <strong
