import PropTypes from 'prop-types';
import { useTranslation } from 'react-i18next';
import {
    Chart as ChartJS,
    CategoryScale,
    LinearScale,
    PointElement,
    LineElement,
    BarElement,
    Title,
    Tooltip,
    Legend,
    Filler,
} from 'chart.js';
import { Line, Bar } from 'react-chartjs-2';

// Register Chart.js components
ChartJS.register(
    CategoryScale,
    LinearScale,
    PointElement,
    LineElement,
    BarElement,
    Title,
    Tooltip,
    Legend,
    Filler
);

/**
 * AnalyticsMultiChart - Reusable multi-series chart component for analytics
 * Supports both line and bar charts with multiple datasets
 * 
 * @param {Object} props
 * @param {Array} props.data - Chart data points
 * @param {Array} props.datasets - Dataset configurations
 * @param {string} props.title - Chart title
 * @param {string} props.subtitle - Chart subtitle
 * @param {string} props.type - Chart type ('line' or 'bar')
 * @param {boolean} props.stacked - Whether to stack datasets
 * @param {boolean} props.showGrid - Whether to show grid lines
 * @param {boolean} props.showLegend - Whether to show legend
 * @param {Object} props.options - Additional Chart.js options
 * @param {string} props.height - Chart height (default: '300px')
 * @param {boolean} props.loading - Whether chart is loading
 * @param {boolean} props.error - Whether chart has error
 * @param {string} props.errorText - Error message to display
 */
const AnalyticsMultiChart = ({
    data = [],
    datasets = [],
    title = '',
    subtitle = '',
    type = 'line',
    stacked = false,
    showGrid = true,
    showLegend = true,
    options = {},
    height = '300px',
    loading = false,
    error = false,
    errorText = '',
}) => {
    const { t } = useTranslation();
    const resolvedErrorText = errorText || t('analytics.common.chartDataLoadError');

    // EduBot color palette
    const colorPalette = [
        {
            border: 'rgb(251, 146, 60)',
            background: 'rgba(251, 146, 60, 0.1)',
            point: 'rgb(251, 146, 60)',
            bar: 'rgba(251, 146, 60, 0.8)',
        },
        {
            border: 'rgb(34, 197, 94)',
            background: 'rgba(34, 197, 94, 0.1)',
            point: 'rgb(34, 197, 94)',
            bar: 'rgba(34, 197, 94, 0.8)',
        },
        {
            border: 'rgb(59, 130, 246)',
            background: 'rgba(59, 130, 246, 0.1)',
            point: 'rgb(59, 130, 246)',
            bar: 'rgba(59, 130, 246, 0.8)',
        },
        {
            border: 'rgb(168, 85, 247)',
            background: 'rgba(168, 85, 247, 0.1)',
            point: 'rgb(168, 85, 247)',
            bar: 'rgba(168, 85, 247, 0.8)',
        },
        {
            border: 'rgb(20, 184, 166)',
            background: 'rgba(20, 184, 166, 0.1)',
            point: 'rgb(20, 184, 166)',
            bar: 'rgba(20, 184, 166, 0.8)',
        },
    ];

    // Transform data for Chart.js
    const chartData = {
        labels: data.map(item => item.label || item.date || item.period || ''),
        datasets: datasets.map((dataset, index) => {
            const color = colorPalette[index % colorPalette.length];
            const datasetData = data.map(item => item[dataset.dataKey] || item.value || 0);

            return {
                label: dataset.label || t('analytics.common.datasetLabel', { number: index + 1 }),
                data: datasetData,
                borderColor: dataset.borderColor || color.border,
                backgroundColor: dataset.backgroundColor ||
                    (type === 'line' ?
                        (dataset.fillArea ? color.background : 'transparent') :
                        color.bar),
                borderWidth: 2,
                pointBackgroundColor: dataset.pointColor || color.point,
                pointBorderColor: '#fff',
                pointBorderWidth: 2,
                pointRadius: 4,
                pointHoverRadius: 6,
                tension: type === 'line' ? 0.4 : 0,
                fill: type === 'line' ? (dataset.fillArea !== false) : false,
                borderRadius: type === 'bar' ? 6 : 0,
                borderSkipped: false,
                hidden: dataset.hidden || false,
            };
        }),
    };

    // Default Chart.js options
    const defaultOptions = {
        responsive: true,
        maintainAspectRatio: false,
        plugins: {
            legend: {
                display: showLegend,
                position: 'top',
                labels: {
                    usePointStyle: true,
                    padding: 20,
                    font: {
                        size: 12,
                        family: 'Inter, sans-serif',
                    },
                    color: 'rgb(107, 114, 128)', // gray-500
                },
            },
            tooltip: {
                backgroundColor: 'rgba(0, 0, 0, 0.8)',
                titleColor: '#fff',
                bodyColor: '#fff',
                padding: 12,
                cornerRadius: 8,
                displayColors: true,
                mode: 'index',
                intersect: false,
                callbacks: {
                    label: (context) => {
                        const value = context.parsed.y;
                        const label = context.dataset.label || '';
                        return `${label}: ${value.toLocaleString()}`;
                    },
                },
            },
        },
        scales: {
            x: {
                grid: {
                    display: showGrid,
                    color: 'rgba(107, 114, 128, 0.1)', // gray-500/10
                },
                ticks: {
                    color: 'rgb(107, 114, 128)', // gray-500
                    font: {
                        size: 11,
                        family: 'Inter, sans-serif',
                    },
                },
                stacked: stacked,
            },
            y: {
                grid: {
                    display: showGrid,
                    color: 'rgba(107, 114, 128, 0.1)', // gray-500/10
                },
                ticks: {
                    color: 'rgb(107, 114, 128)', // gray-500
                    font: {
                        size: 11,
                        family: 'Inter, sans-serif',
                    },
                    callback: (value) => value.toLocaleString(),
                },
                stacked: stacked,
                beginAtZero: true,
            },
        },
        interaction: {
            intersect: false,
            mode: 'index',
        },
        elements: {
            point: {
                hoverBorderWidth: 3,
            },
        },
        ...options,
    };

    // Loading state
    if (loading) {
        return (
            <div className="w-full" style={{ height }}>
                <div className="flex items-center justify-center h-full bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                    <div className="text-center">
                        <div className="animate-spin rounded-full border-2 border-gray-300 border-t-edubot-orange w-8 h-8 mx-auto mb-3"></div>
                        <p className="text-sm text-gray-600 dark:text-gray-400">{t('common.loading')}...</p>
                    </div>
                </div>
            </div>
        );
    }

    // Error state
    if (error) {
        return (
            <div className="w-full" style={{ height }}>
                <div className="flex items-center justify-center h-full bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                    <div className="text-center">
                        <svg className="w-12 h-12 text-gray-400 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.732-.833-2.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z" />
                        </svg>
                        <p className="text-sm text-gray-600 dark:text-gray-400">{resolvedErrorText}</p>
                    </div>
                </div>
            </div>
        );
    }

    // Empty state
    if (!data || data.length === 0) {
        return (
            <div className="w-full" style={{ height }}>
                <div className="flex items-center justify-center h-full bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                    <div className="text-center">
                        <svg className="w-12 h-12 text-gray-400 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                        </svg>
                        <p className="text-sm text-gray-600 dark:text-gray-400">{t('analytics.common.noData')}</p>
                    </div>
                </div>
            </div>
        );
    }

    const ChartComponent = type === 'bar' ? Bar : Line;

    return (
        <div className="w-full">
            {(title || subtitle) && (
                <div className="mb-4">
                    {title && (
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                            {title}
                        </h3>
                    )}
                    {subtitle && (
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                            {subtitle}
                        </p>
                    )}
                </div>
            )}
            <div style={{ height }}>
                <ChartComponent data={chartData} options={defaultOptions} />
            </div>
        </div>
    );
};

AnalyticsMultiChart.propTypes = {
    data: PropTypes.array,
    datasets: PropTypes.array,
    title: PropTypes.string,
    subtitle: PropTypes.string,
    type: PropTypes.oneOf(['line', 'bar']),
    stacked: PropTypes.bool,
    showGrid: PropTypes.bool,
    showLegend: PropTypes.bool,
    options: PropTypes.object,
    height: PropTypes.string,
    loading: PropTypes.bool,
    error: PropTypes.bool,
    errorText: PropTypes.string,
};

export default AnalyticsMultiChart;
