  ...
  if (error) {
    return (
      <div className={`text-center py-8 ${className}`}>
        ...
        <p className="text-gray-500 dark:text-gray-400">{t('analytics.common.dataLoadError')}</p>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className={`rounded-2xl border border-dashed border-edubot-line/80 bg-edubot-surfaceAlt/40 py-8 text-center dark:border-slate-700 dark:bg-slate-900/60 ${className}`}>
        ...
        <p className="text-edubot-muted dark:text-slate-400">{resolvedEmptyMessage}</p>
      </div>
    );
  }
